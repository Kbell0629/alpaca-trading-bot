...
richer 5-tier composite regime from SPY 200-MA + breadth + VIX,
then applies per-strategy weights tuned for each regime.
  * **`screener_core.REGIME_WEIGHTS`** — table mapping each of five
    composite regimes (strong_bull / weak_bull / choppy / weak_bear
    / strong_bear) to per-strategy multipliers.
  * **`screener_core.compute_composite_regime(...)`** — pure regime
    classifier. Missing inputs → `choppy` default.
  * **`screener_core.apply_regime_weighting(picks, regime)`** —
    multiplies per-strategy scores, recomputes `best_score`,
    tags every pick. Re-sorts.
  * Wired into `update_dashboard.py` after `apply_factor_scores`.
+27 tests in `tests/test_round61_pt42_regime_weighting.py`.

**Pt.41 landed (PR #168):** per-strategy adaptive thresholds.
Self-correcting score multipliers based on rolling 30-trade win
rate. Cold strategies (<40%) demoted ×0.70, hot strategies (>60%)
boosted ×1.30. +29 tests.

**Pt.40 landed (PR #167):** multi-day breakout confirmation. Per
Breakout-strategy pick, requires today's close > today's 20-day
high AND yesterday's close > prior 20-day high. Single-day
breakouts get score halved. +22 tests.
